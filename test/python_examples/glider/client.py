...
class Client:
    ...
    def __create_random_pos(pos):
        ...
        rp = {
            ...
            'depth': 123.456, # dummy value
            'qc_flag': 'dummy qc_flag for pos'
        }
        if pos == "all":
            return rp
    ...
    def __create_timeseries(self, frost_api_base, dset):
        url = '{}/api/v1/obs/glider/ts/create'.format(frost_api_base)

        # remove/empty all 'observations' objects from the dataset ... TBD
        dset2 = self.__remove_observations(dset)

        print('creating time series in Frost/glider ({}) ...'.format(url))
    ...
    def __upload_dataset(frost_api_base, dset):
        url = '{}/api/v1/obs/glider/put'.format(frost_api_base)

        print('uploading to Frost/glider ({}) ...'.format(url))
    ...
    def __download_dataset(frost_api_base, tsid, t1, t2):
        ...
        url = '{}/api/v1/obs/glider/get?hdrmatch={}&time={}/{}&incobs=true'.format(
            frost_api_base, hdrmatch, t1, t2)

        print('downloading from Frost/glider ({}) ...'.format(url))
        r = requests.get(url)
    ...
    def __header_ids_equivalent(self, id1, id2):
        ...
        if source1 != source2:
            return False
        gliderid1 = id1['gliderid'] if 'gliderid' in id1 else None
        gliderid2 = id2['gliderid'] if 'gliderid' in id2 else None
        if gliderid1 != gliderid2:
            return False
